...
import "../styles/ProductCardFav.css";
import ModalProduct from "../Modals/ModalProduct";
import swal from "sweetalert";
const db = getFirestore(app);

const ProductCardFav = ({ prod }) => {
  const { user } = useAuth();
  ...
  const handleLike = async () => {
    ...
    if (user === null) {
      swal(
        "¡Ops algo ocurrió!",
        "Debes iniciar sesion para poder dar me gusta, si no tienes una cuenta, puedes crear una en registrate en el menu"
      );
    } else {
      const q = query(collection(db, "userName"));
      const querySnapshot = await getDocs(q);
      const queryData = querySnapshot.docs.map((data) => ({
        ...data.data(),
        id: data.id,
      }));
      ...
      // debugger
      queryData.map(async () => {
        await deleteDoc(doc(db, `userName/${user.uid}/favProduct`, prod.id), {
          name: prod.name,
          price: prod.price,
    ...
    <li className="product__container">
      ...
      <ModalProduct open={isOpen}>
        ...
        <div className="product__name--modal">
          ...
          <p onClick={() => setIsOpen(false)} className="product__close">
            cerrar
          </p>
        </div>
      </ModalProduct>
